...
const AiQuestions = () => {
    ...
    const togglePinApi = async (id) => {
        const res = await axiosInstance.patch(`/question/${id}/pin`); // ✅ fixed endpoint and method
        return res.data;
    };
    ...
    if (isLoading) {
        return (
            <div className="w-full flex items-center justify-center py-20">
                <h1 className="text-xl md:text-2xl font-semibold text-gray-300">Loading...</h1>
            </div>
        );
    }
    ...
    return (
        <LayoutGroup>
            <div className="mt-10 w-[60vw] mr-2.5 flex flex-col items-center gap-6">
                <AnimatePresence>
                    {data?.data?.session?.questions?.map((item) => ( // ✅ fixed data path
                        <motion.div
                            key={item._id}
                            layout
                   ...
                   hover:scale-[1.02] transition-all duration-300 shadow-md cursor-pointer"
                                    ...
                                    <AccordionContent className="w-full bg-[#e0f4ff]/20 text-gray-100 rounded-2xl p-4 mt-2 shadow-inner">
                                        <p className="text-sm md:text-base lg:text-lg">{item?.answer}</p>
                                    </AccordionContent>
                                </AccordionItem>
                            </Accordion>
                        </motion.div>
